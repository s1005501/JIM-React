    ...
    <div>
      <div className="O_Process_One_Sort">
        <div className="O_Process_One_Margin">
          ...
          <p>工作室 : {orderInfoLocalStorage.storeName}</p>
          <p>地址 : {orderInfoLocalStorage.storeAddress}</p>
          <p>預約日期 : {orderInfoLocalStorage.date}</p>
          <p>預約時間 : {orderInfoLocalStorage.time}</p>
          <p>遊玩時間 : {orderInfoLocalStorage.gameTime} 分鐘</p>
          <p>遊玩人數 : {orderInfoLocalStorage.people} 人</p>
          <p className="O_Process_One_Sum">
            總金額 : {orderInfoLocalStorage.price} 元
          </p>
        </div>
        <div style={{ width: '300px' }}>
          <img
            // src={`/Images/orders/${orderInfoLocalStorage.gamesImages}`}
            src={`/Images/orders/${orderInfoLocalStorage.gamesLogo}`}
            alt=""
            className="O_Process_One_GameImg"
          />
        </div>
